...
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 600
CAPTION = "Hex Game - Victory Path & Border Fixed"
HEX_RADIUS = 30 
...
RED = (200, 0, 0)
BLUE = (0, 0, 200)
GOLD = (255, 215, 0) # Highlight color for victory path
DARK_GRAY = (40, 40, 40)
    ...
    while running:
            ...
            for r in range(board_size):
                for c in range(board_size):
                    ...
                    if c == 0:
                        ...
                        pygame.draw.line(screen, BLUE, verts[3], verts[4], b_thick)
                    if c == board_size - 1:
                        pygame.draw.line(screen, BLUE, verts[5], verts[0], b_thick)
                        pygame.draw.line(screen, BLUE, verts[0], verts[1], b_thick)
                    ...
                    if (r, c) in win_path:
                        pygame.draw.polygon(screen, GOLD, verts, 5) # Thick Gold outline for win
                    else:
        ...
        if game_mode is None:
            for val in modes.values():
                ...
                txt = font_small.render(val['text'], True, WHITE)
                screen.blit(txt, txt.get_rect(center=val['rect'].center))
        else:
